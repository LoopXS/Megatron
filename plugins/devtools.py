...
async def doie(e):
    ...
    open("cpp-cipherx.cpp", "w").write(match)
    m = await bash("g++ -o CppCipherX cpp-cipherx.cpp")
    o_cpp = f"• **Eval-Cpp**\n`{match}`"
    if m[1] != "":
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
            os.remove("cpp-cipherx.cpp")
            if os.path.exists("CppCipherX"):
                os.remove("CppCipherX")
